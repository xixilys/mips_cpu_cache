...
class cfu extends Module with mips_macros {
    ...
    val io = IO(new Bundle { 
        ...
        val    ForwardAD = Output(UInt(2.W))
        val    ForwardBD = Output(UInt(2.W))
    ...
    io.ForwardAD := Mux(io.RsD === 0.U,0.U ,Mux(io.RsD === io.WriteRegM && io.RegWriteM.asBool && !io.MemToRegM.asBool,"b01".U,Mux(
        io.RsD === io.WriteRegM2 && io.RegWriteM2.asBool && !io.MemToRegM2.asBool,"b10".U,0.U)))//当Rs为操作数的时候将该寄存器前递
    io.ForwardBD := Mux(io.RtD === 0.U,0.U ,Mux(io.RtD === io.WriteRegM && io.RegWriteM.asBool && !io.MemToRegM.asBool,"b01".U,Mux(
        io.RtD === io.WriteRegM2 && io.RegWriteM2.asBool && !io.MemToRegM2.asBool,"b10".U,0.U)))//当Rs为操作数的时候将该寄存器前递

    //exe阶段与
    

    io.ForwardAE := Mux(io.RsE === 0.U,0.U,MuxCase(0.U,Seq(
        (io.RsE === io.WriteRegM && io.RegWriteM.asBool && !io.MemToRegM.asBool)    -> "b10".U,
        (io.RsE === io.WriteRegM2 && io.RegWriteM2.asBool && !io.MemToRegM2.asBool) -> "b11".U,
        (io.RsE === io.WriteRegW && io.RegWriteW.asBool)    -> "b01".U

    )))

    io.ForwardBE := Mux(io.RtE === 0.U,0.U,MuxCase(0.U,Seq(
        (io.RtE === io.WriteRegM && io.RegWriteM.asBool && !io.MemToRegM.asBool)    -> "b10".U,
        (io.RtE === io.WriteRegM2 && io.RegWriteM2.asBool && !io.MemToRegM2.asBool) -> "b11".U,
        (io.RtE === io.WriteRegW && io.RegWriteW.asBool )    -> "b01".U

    )))
        // Mux(io.RsE =/= 0.U && io.RsE === io.WriteRegM && io.RegWriteM.asBool && !io.MemToRegM.asBool,"b10".U,Mux(
    ...
    io.ForwardCP0E := Mux(io.CP0ToRegE.asBool,MuxCase(0.U,Seq(
        ...
        // Mux((io.HiLoToRegE & io.HiLoWriteM) =/= 0.U,"b10".U,Mux(
...
//感觉lm stall不是很有用，直接exe阶段前递就可以了来着
    ...
    val br_Stall = (io.CanBranchD.asBool && (io.BranchD_Flag .asBool) && 
        ...
        (io.MemToRegM.asBool && (io.WriteRegM === io.RsD || io.WriteRegM === io.RtD)) || 
        (io.MemToRegM2.asBool && (io.WriteRegM2 === io.RsD || io.WriteRegM2 === io.RtD)))) && !io.InException.asBool //需要从mem读的寄存器冲突了
    val jr_Stall = ( io.JRD.asBool || io.dmem_calD.asBool) && 
        ((io.RegWriteE.asBool && (io.WriteRegE === io.RsD)) || //直接要写的寄存器和冲突了
        (io.MemToRegM.asBool && (io.WriteRegM === io.RsD ) ) || 
        (io.MemToRegM2.asBool && (io.WriteRegM2 === io.RsD )))  && !io.InException.asBool //需要从mem读的寄存器冲突了

    val dmem_addr_cal_Stall = jr_Stall//(io.dmem_calD) && 
        // ((io.RegWriteE.asBool && (io.WriteRegE === io.RtD)) || //直接要写的寄存器和冲突了
        // (io.MemToRegM.asBool && (io.WriteRegM === io.RtD ) ) || 
        // (io.MemToRegM2.asBool && (io.WriteRegM2 === io.RtD )))  && !io.InException.asBool //需要从mem读的寄存器冲突了
    ...
    val mem2regM_Stall = (/*io.RsE =/= 0.U && */io.RsE === io.WriteRegM && io.RegWriteM.asBool && io.MemToRegM.asBool) || 
                ...
                (/*io.RsE =/= 0.U &&*/ io.RsE === io.WriteRegM2 && io.RegWriteM2.asBool && io.MemToRegM2.asBool) || 
                (/*io.RtE =/= 0.U &&*/ io.RtE === io.WriteRegM2 && io.RegWriteM2.asBool && io.MemToRegM2.asBool) ||//mem阶段出现mem2reg并且此时需要前递时，停止流水线
                (/*io.RsD =/= 0.U &&*/ io.RsD === io.WriteRegM2 && io.RegWriteM2.asBool && io.MemToRegM2.asBool) || 
                (/*io.RtD =/= 0.U &&*/ io.RtD === io.WriteRegM2 && io.RegWriteM2.asBool && io.MemToRegM2.asBool) 
